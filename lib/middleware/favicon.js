...
var fs = require('fs')
  ...
  , debug = require('debug')('voyager:middleware:favicon')
  , _ = require('../_');
...
Favicon.prototype.handle = function (req, res, next) {
  ...
  debug('favicon requested');

  if (this.icon) {
    debug('retrieving cached favicon');

    res.writeHead(200, self.icon.headers);
    res.end(self.icon.body);
  } else {

    debug('looking for favicon in %s', this.options.path);
